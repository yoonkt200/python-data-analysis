...
- 4쇄 부터 반영되는 변경 내용
    - 공통 사항
        - 책에 기재된 크롤링 관련 예제들에 언급되어 있는 페이지들이 일반적인 방식의 크롤링을 기술적으로 차단함에 따라, 페이지를 크롤링하기 위한 난이도가 책에서 다룰 수 있는 난이도를 크게 벗어났습니다.
        - 따라서 기재된 크롤링 예제를 대신하여, 현재(2023년 3월 기준) 일반적인 방식의 크롤링이 가능한 예제들로 변경하여 현 저장소에 업로드 해두었습니다.
        - 크롤링의 방식이나 크롤링된 데이터로 적용되는 데이터 분석의 내용은 동일하기 때문에, 출판사와 협의 하에 개정판은 내지 않는 것으로 결정하였습니다. 대신 4쇄 증쇄본에서는 github 공지에 바뀐 크롤링 페이지 예제를 올리는 것으로 결정하였습니다. 이 부분 독자 여러분들의 양해 부탁드립니다.
    - 2장의 나무위키 예제
        - 페이지 하단의 버전 업데이트 정보(Release Note)를 참고해주세요.
    - 4장의 감성 분석 예제
        - 페이지 하단의 버전 업데이트 정보(Release Note)를 참고해주세요.
- 3쇄 부터 반영되는 변경 내용
    - ~~2장의 나무위키 예제~~
...
- v3.0.0 (2023.02.27) : 인쇄 4쇄부터 반영되는 내용
    - 2장의 나무위키 예제
...
- v2.0.0 (2021.06.16) : 인쇄 3쇄부터 반영되는 내용
    - 2장의 나무위키 예제
        - 기존에 requests, cloudscraper를 사용하던 정적 크롤링 방식 대신, selenium과 chromediver를 사용하는 동적 크롤링 방식으로 변경되었습니다.
        - 코드 내용에 큰 변화는 없습니다.
        - 이에 따라 책의 내용도 약간 변경되었습니다.
...
- v1.0.0 (2020.02.27)
    - 2020.02.10 출간 직후 ~ 현재까지의 업데이트 버전입니다
-----
### **How to contribute**
